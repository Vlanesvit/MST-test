...
let formValidate = {
	getErrors(form) {
		let error = 0;
		let formRequiredItems = form.querySelectorAll('input,textarea,select');
		formRequiredItems.forEach(formRequiredItem => {
			if ((formRequiredItem.offsetParent !== null || formRequiredItem.tagName === "SELECT") && !formRequiredItem.disabled) {
				if (!formRequiredItem.hasAttribute('data-required') && !formRequiredItem.value.trim()) {
					this.addError(formRequiredItem);
					error++;
				} else if (formRequiredItem.hasAttribute('data-required')) {
